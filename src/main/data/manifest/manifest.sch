...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt3"
  ...
  <sch:pattern>
    ...
    <sch:rule context="cx:witness/@url">
      ...
      <!-- TODO: Use 4.0 parse-uri() when available to manage fragment    -->
      <!--   identifiers and query strings                                -->
      <!-- ============================================================== -->
      <sch:let name="my-resolved-uri" value="resolve-uri(., base-uri(/))"/>
      <sch:let name="preceding-resolved-same-uris"
        value="(../preceding-sibling::cx:witness/@url ! resolve-uri(., base-uri(/)))[. eq $my-resolved-uri]"/>
      <sch:let name="following-resolved-same-uris-pos"
        value="../following-sibling::cx:witness[resolve-uri(@url, base-uri(/)) eq $my-resolved-uri] ! (count(preceding-sibling::cx:witness) + 1)"/>
      <sch:report
      ...
      <sch:report
        ...
        >The current @url (<sch:value-of select="."/>) is duplicated in the witnesses at offsets
          <sch:value-of select="string-join($following-resolved-same-uris-pos, ', ')"
        />.</sch:report>
      <!-- ============================================================== -->
      <!-- Verify that url can be reached.                                -->
      <!-- ============================================================== -->
      <sch:assert test="unparsed-text-available(.)">The url <sch:value-of select="."/> cannot be
        resolved.</sch:assert>
    </sch:rule>
  </sch:pattern>
</sch:schema>
