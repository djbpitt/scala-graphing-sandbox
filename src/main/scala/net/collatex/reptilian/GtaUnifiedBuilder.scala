...
//    cfg: GtaUnifiedBuilder.BuildConfig
//): Either[String, Seq[WitnessData]] = {
//  val rootFontOpt = manifest.obj.value.get("font").map(_.str)
//  val witnesses = manifest("witnesses").arr.toSeq
//  var gCounter = 0
//  val result = scala.collection.mutable.ListBuffer.empty[WitnessData]
//
//  witnesses.zipWithIndex.foldLeft((Vector.empty[WitnessData], gCounter)) { (acc, current) =>
//    val (currentWitInfo, currentWitOffset): (Value, Int) = current
//    val siglum = currentWitInfo("id").str
//    val witnessFont = currentWitInfo.obj.value.get("font").map(_.str)
//    val finalFont = witnessFont.orElse(rootFontOpt)
//    if (currentWitInfo.obj.contains("content")) {
//      val content = currentWitInfo("content").str
//      val ts: Iterator[String] = // raw token strings (t values)
...
//          gCounter + acc._1.map(_.tokens.size).sum + currentWitOffset
//        ) // currentWitOffset is, coincidentally and helpfully, also a count of TokenSep instances
//      val currentColor = currentWitInfo.obj.get("color") match {
//        case Some(c) => c.toString
//        case None => GtaUnifiedBuilder.defaultColors(currentWitOffset % defaultColors.length)
//      }
//      result += WitnessData(Siglum(siglum), currentColor, finalFont, content)
