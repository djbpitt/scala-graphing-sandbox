...
object DisplayFunctions {
  ...
  /** Horizontal plain text table; rows as witnesses, columns as alignment points
    ...
    */
  private[display] def emitTableHorizontal(
  ...
  /** Vertical plain text table; rows as alignment points, columns as witnesses
    ...
    */
  private[display] def emitTableVertical(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      gTa: Vector[TokenEnum],
      outputBaseFilename: Set[String]
  ...
  /** Horizontal HTML table; rows as witnesses, columns as alignment points
    ...
    */
  private[display] def emitTableHorizontalHTML(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      gTa: Vector[TokenEnum],
  ...
  /** Vertical HTML table; rows as alignment points, columns as witnesses
    ...
    *   None; writes HTML document to filesystem or stdout
    */
  private[display] def emitTableVerticalHTML(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      gTa: Vector[TokenEnum],
      outputBaseFilename: Set[String], // validated: empty or exactly one value
  ...
  /** Create alignment ribbon visualization (HTML and SVG)
    ...
    */
  private[display] def emitAlignmentRibbon(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      displayColors: List[String],
      fonts: List[Option[String]],
  ...
  /** Create SVG representation of Rhine delta (formerly variant graph)
    ...
    *   List of Sigla in output order (List[Siglum])
    * @param outputBaseFilename
    *   Base filename for file-system output. If empty, output goes to stdout. If present, '.svg' is appended to
    *   construct the output filename, e.g., 'foo' becomes 'foo.svg'.
    * @param rich
...
>]
   ...
   */
  private[display] def emitSvgGraph(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
  ...
  /** GraphML output
    ...
    */
  private[display] def emitGraphMl(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      outputBaseFilename: Set[String]
  ): Unit =
  ...
  /** Helper to create edges for GraphML output
    ...
    */
  private[display] def createGraphMlEdges(
  ...
  /** TEI parallel segmentation
    ...
    * corresponding to where a human tagger would have put a shared inter-token space character. This adds an extra
    * space in the case of missing witnesses, which can be flattened, if desired, in post-processing or by
    * pretty-printing.
    *
    * @param alignment
    *   AlignmentRibbon; children property is a ListBuffer of AlignmentPoint instances (but defined as AlignmentUnit)
    ...
    *   None. Write XML document to filesystem or stdout
    */
  private[display] def emitTeiXml(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      outputBaseFilename: Set[String]
  ): Unit = {
  ...
  /** Pretty-print XML using Saxon
    ...
    */
  private val saxonPrettyPrinter: String => String = {
  ...
  /** Custom CollateX XML structure
    ...
    * @param outputBaseFilename
    *   Base filename for file-system output. If empty, output goes to stdout. If populated, must be singleton, and
    *   '.xml' is appended to construct the output filename, e.g., 'foo' becomes 'foo.xml'.
    *
    * @return
    *   None. Write XML document to filesystem or stdout
    */
  private[display] def emitXml(
      alignment: AlignmentRibbon,
      displaySigla: List[Siglum],
      outputBaseFilename: Set[String]
  ): Unit =
  ...
  /** Node data for GraphML output
    ...
    */
  private[display] case class GraphMlNodeProperties(
      number: String,
      content: String,
      rank: Int,
      witIds: Set[WitId]
